...
class SEAdvancedEncoder {
    ...
    func save(toFile file: URL) -> Bool {
        let audioFile = try? AVAudioFile(forWriting: file, settings: [AVSampleRateKey: NSNumber(floatLiteral: sampleRate), AVNumberOfChannelsKey: 1], commonFormat: .pcmFormatFloat32, interleaved: false)
        do {
    ...
    func synthesize() -> AVAudioPCMBuffer {
